...
void ADARSH();			// 绕过游戏保护
// void RecADARSH(char *bm);//解除(停止使用)
...
/* int getPID(PACKAGENAME bm[64]) { FILE *fp;//文件指针 pid_t pid;//pid
   char log[64];//命令 sprintf(log,"pidof %s > log.txt",bm);
   system(log);//执行 if ((fp = fopen("log.txt", "r")) == NULL) { return -1;
   } fscanf(fp, "%d", &pid);//读取pid remove("log.txt"); return pid; } */
...
PMAPS BaseAddressSearch_DWORD(PACKAGENAME * bm, int value, COUNT * gs, ADDRESS BaseAddr,
							  PMAPS pMap)
{
	*gs = 0;
...
PMAPS BaseAddressSearch_FLOAT(PACKAGENAME * bm, float value, COUNT * gs, ADDRESS BaseAddr,
							  PMAPS pMap)
{
	*gs = 0;
	pid_t pid = getPID(bm);
...
/* void RecADARSH(char *bm) { char ml[80]; sprintf(ml,"chmod 771
   /data/data/%s/files",bm); system(ml); } */
...
int PrintFreezeItems()
{
	...
	for (int i = 0; i < FreezeCount; i++)
	{
		// printf("FreezeAddr:%lx,type:%s,value:%s\n",pTemp->addr,pTemp->type
		// == DWORD ? "DWORD":"FLOAT",pTemp->value);
		pTemp = pTemp->next;
	}
	return 0;
}
